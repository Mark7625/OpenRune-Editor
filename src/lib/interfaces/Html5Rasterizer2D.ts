...
export class Html5Rasterizer2D {
    ...
    constructor(ctx: CanvasRenderingContext2D) {
        ...
        const textCtx = this.ctx as CanvasRenderingContext2D & {
    ...
    setClip(x1: number, y1: number, x2: number, y2: number) {
        x1 = Html5Rasterizer2D.snap(x1);
        y1 = Html5Rasterizer2D.snap(y1);
        x2 = Html5Rasterizer2D.snap(x2);
        y2 = Html5Rasterizer2D.snap(y2);
    ...
    drawHLine(x: number, y: number, len: number, color: number) {
        x = Html5Rasterizer2D.snap(x);
        y = Html5Rasterizer2D.snap(y);
        len = Html5Rasterizer2D.snapSize(len);
        ...
        if (len <= 0) return;
        this.ctx.fillStyle = Html5Rasterizer2D.toRgb(color);
    ...
    fillRect(x: number, y: number, w: number, h: number, color: number) {
        x = Html5Rasterizer2D.snap(x);
        y = Html5Rasterizer2D.snap(y);
        w = Html5Rasterizer2D.snapSize(w);
        h = Html5Rasterizer2D.snapSize(h);
        this.ctx.fillStyle = Html5Rasterizer2D.toRgb(color);
    ...
    fillRectAlpha(x: number, y: number, w: number, h: number, color: number, alpha256: number) {
        ...
        w = Html5Rasterizer2D.snapSize(w);
        h = Html5Rasterizer2D.snapSize(h);
        this.ctx.fillStyle = Html5Rasterizer2D.toRgba(color, alpha256);
        this.ctx.fillRect(x, y, w, h);
    }
    ...
    fillRectGradient(
        ...
        grad.addColorStop(0, Html5Rasterizer2D.toRgb(colorTop));
        grad.addColorStop(1, Html5Rasterizer2D.toRgb(colorBottom));
    ...
    fillRectGradientAlpha(
        ...
        h = Html5Rasterizer2D.snapSize(h);
        const grad = this.ctx.createLinearGradient(x, y, x, y + h);
        grad.addColorStop(0, Html5Rasterizer2D.toRgba(colorTop, alphaTop));
        grad.addColorStop(1, Html5Rasterizer2D.toRgba(colorBottom, alphaBot));
        this.ctx.fillStyle = grad;
        this.ctx.fillRect(x, y, w, h);
    }
    ...
    drawLine(x1: number, y1: number, x2: number, y2: number, color: number) {
        ...
        x2 = Html5Rasterizer2D.snap(x2);
        y2 = Html5Rasterizer2D.snap(y2);
        this.ctx.strokeStyle = Html5Rasterizer2D.toRgb(color);
    ...
    drawThickLine(
        ...
        x2 = Html5Rasterizer2D.snap(x2);
        y2 = Html5Rasterizer2D.snap(y2);
        thickness = Math.max(1, Html5Rasterizer2D.snap(thickness));
        const dx = x2 - x1;
        const dy = y2 - y1;
    ...
    drawScrollBar(
        ...
        viewHeight: number,
        scrollHeight: number,
        sprites: { up: SpritePixels | null; down: SpritePixels | null } | null = null,
    ) {
        ...
        this.fillRectangle(var0, var1, cell, var3, Html5Rasterizer2D.SCROLL_BAR_BG);

        //this._drawScrollBarArrowSprite(up, var0, var1, cell);
        //this._drawScrollBarArrowSprite(down, var0, var3 + var1 - cell, cell);

        this.fillRectangle(var0, var1 + cell, cell, trackLen, Html5Rasterizer2D.SCROLL_TRACK_MID);
        ...
        this.fillRectangle(var0, thumbY, cell, var5, Html5Rasterizer2D.SCROLL_THUMB_FILL);

        this.drawVerticalLine(var0, thumbY, var5, Html5Rasterizer2D.SCROLL_FIELD568);
        this.drawVerticalLine(var0 + 1, thumbY, var5, Html5Rasterizer2D.SCROLL_FIELD568);
        this.drawHorizontalLine(var0, thumbY, cell, Html5Rasterizer2D.SCROLL_FIELD568);
        this.drawHorizontalLine(var0, thumbY + 1, cell, Html5Rasterizer2D.SCROLL_FIELD568);
        this.drawVerticalLine(var0 + 15, thumbY, var5, Html5Rasterizer2D.SCROLL_FIELD567);
        this.drawVerticalLine(var0 + 14, thumbY + 1, var5 - 1, Html5Rasterizer2D.SCROLL_FIELD567);
        this.drawHorizontalLine(
            var0,
            var5 + var6 + var1 + 15,
            cell,
            Html5Rasterizer2D.SCROLL_FIELD567,
        );
        this.drawHorizontalLine(
            var0 + 1,
            var6 + var5 + var1 + 14,
    ...
    private _drawScrollBarArrowSprite(
        spr: SpritePixels,
        cellX: number,
        cellY: number,
        cell: number,
    ): void {
        const ax = cellX + Math.trunc((cell - spr.subWidth) / 2) - spr.xOffset;
        const ay = cellY + Math.trunc((cell - spr.subHeight) / 2) - spr.yOffset;
    ...
    private _drawArrow(x: number, y: number, size: number, up: boolean) {
        ...
        this.ctx.fillStyle = Html5Rasterizer2D.toRgb(Html5Rasterizer2D.SCROLL_FIELD568);
        this.ctx.beginPath();
        if (up) {
    ...
    drawText(
        ...
        for (let li = 0; li < lines.length; li++) {
            ...
            const baselineY = Html5Rasterizer2D.snap(topY + li * effectiveLineH);

            for (const run of line) {
                if (!run.text) continue;
                this.ctx.font = `${run.italic ? "italic " : ""}${run.bold ? "700 " : ""}${
                    base.sizePx
                }px ${base.family}`;
                this.ctx.fillStyle = Html5Rasterizer2D.toRgb(run.color);
                const drawX = Html5Rasterizer2D.snap(tx);
                this.ctx.fillText(run.text, drawX, baselineY);

                const runWidth = this.ctx.measureText(run.text).width;
                if (run.underlineColor !== null) {
                    this.ctx.fillStyle = Html5Rasterizer2D.toRgb(run.underlineColor);
                    this.ctx.fillRect(
                        drawX,
                        baselineY + base.sizePx - 1,
                        Math.max(1, Math.ceil(runWidth)),
                        1,
                    );
                }
                if (run.strikeColor !== null) {
                    this.ctx.fillStyle = Html5Rasterizer2D.toRgb(run.strikeColor);
                    this.ctx.fillRect(
                        drawX,
                        baselineY + Math.floor(base.sizePx / 2),
                        Math.max(1, Math.ceil(runWidth)),
                        1,
                    );
                }
    ...
    private _resolveOsrsFont(fontId?: number): { family: string; sizePx: number } {
        const fallback = Html5Rasterizer2D.FONT_BY_ID[494]!;
        if (fontId == null || !Number.isFinite(fontId)) {
            const key = "missing";
            if (!Html5Rasterizer2D.warnedFontIds.has(key)) {
                Html5Rasterizer2D.warnedFontIds.add(key);
                console.warn(
                    "[interface-renderer] Missing widget textFont id/size; defaulting to Plain 11 (id 494).",
                );
            }
            return fallback;
        }
        const mapped = Html5Rasterizer2D.FONT_BY_ID[fontId];
        if (mapped) return mapped;
        const key = `unknown-${fontId}`;
        if (!Html5Rasterizer2D.warnedFontIds.has(key)) {
            Html5Rasterizer2D.warnedFontIds.add(key);
            console.warn(
                `[interface-renderer] Unknown widget textFont id ${fontId}; defaulting to Plain 11 (id 494).`,
            );
        }
        return fallback;
    }
