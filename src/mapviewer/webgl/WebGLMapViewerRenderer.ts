...
const APP_BACKGROUND_CLEAR = 0.09;
...
export class WebGLMapViewerRenderer extends MapViewerRenderer<WebGLMapSquare> {
    ...
    async init(): Promise<void> {
        ...
        this.app.clearColor(APP_BACKGROUND_CLEAR, APP_BACKGROUND_CLEAR, APP_BACKGROUND_CLEAR, 1.0);
    ...
    checkInteractions(interactions: Interactions): void {
        ...
        let hoverTargetName = "";
        let hoverTargetType = MenuTargetType.NONE;
        let hoverTargetId = -1;

        const getHoverName = (name: string, type: MenuTargetType, id: number): string => {
            if (name !== "null") {
                return name;
            }
            return `${MenuTargetType[type]}#${id}`;
        };
        ...
        for (let i = 0; i < INTERACTION_RADIUS + 1; i++) {
            ...
            for (const index of indices) {
                ...
                if (interactType === InteractType.LOC) {
                    ...
                    if (!hoverTargetName) {
                        hoverTargetName = getHoverName(locType.name, MenuTargetType.LOC, locType.id);
                        hoverTargetType = MenuTargetType.LOC;
                        hoverTargetId = locType.id;
                    }
                    ...
                    if (!hoverTargetName) {
                        hoverTargetName = getHoverName(objType.name, MenuTargetType.OBJ, objType.id);
                        hoverTargetType = MenuTargetType.OBJ;
                        hoverTargetId = objType.id;
                    }
                    ...
                    if (!hoverTargetName) {
                        hoverTargetName = getHoverName(npcType.name, MenuTargetType.NPC, npcType.id);
                        hoverTargetType = MenuTargetType.NPC;
                        hoverTargetId = npcType.id;
                    }
        ...
        if (hoverTargetName) {
            this.mapViewer.debugText = `Hover: ${hoverTargetName} type:${MenuTargetType[hoverTargetType]} id:${hoverTargetId}`;
        }

        const hoverObjectCount = locIds.size + objIds.size + npcIds.size;
        if (hoverObjectCount > 0) {
            menuEntries.push({
                option: `Inspect tile objects (${hoverObjectCount})`,
                targetId: hoverTargetId,
                targetType: hoverTargetType,
                targetName: hoverTargetName,
                targetLevel: -1,
                onClick: (_entry) => {
                    this.mapViewer.debugText = `Tile objects: ${hoverObjectCount}`;
                    this.mapViewer.closeMenu();
                },
            });
        }
