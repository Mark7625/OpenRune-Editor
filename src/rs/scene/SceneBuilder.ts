...
export class SceneBuilder {
    ...
    blendUnderlays(
        ...
        const blendRadius = SceneBuilder.BLEND_RADIUS;

        const blendStartX = -blendRadius;
        const blendStartY = -blendRadius;
        const blendEndX = scene.sizeX + blendRadius;
        const blendEndY = scene.sizeY + blendRadius;

        for (let xi = blendStartX; xi < blendEndX; xi++) {
            for (let yi = 0; yi < scene.sizeY; yi++) {
                const xEast = xi + blendRadius;
    ...
    addTileModels(scene: Scene, smoothUnderlays: boolean): void {
        ...
        for (let level = 0; level < scene.levels; level++) {
            ...
            const blendedColors = this.blendUnderlays(scene, level, smoothUnderlays);
    ...
    /**
     * Like {@link addTileModels} but only rebuilds tile models inside an inclusive scene-tile rectangle
     * (used for incremental minimap patches after edits).
     */
    addTileModelsInRegion(
        scene: Scene,
        ...
        const minX = Math.max(1, Math.min(minTileX, maxTileX));
        const maxX = Math.min(scene.sizeX - 2, Math.max(minTileX, maxTileX));
        const minY = Math.max(1, Math.min(minTileY, maxTileY));
        const maxY = Math.min(scene.sizeY - 2, Math.max(minTileY, maxTileY));
